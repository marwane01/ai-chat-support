...
load_dotenv()
pg = f"postgresql://{os.getenv('POSTGRES_USER','chatbi')}:{os.getenv('POSTGRES_PASSWORD','chatbi')}@" \
     f"{os.getenv('POSTGRES_HOST','localhost')}:{os.getenv('POSTGRES_PORT','5432')}/{os.getenv('POSTGRES_DB','chatbi')}"
engine = create_engine(pg, future=True)

# -------- utils --------
def _norm_row(row: dict) -> dict:
    return { (k or "").replace("\ufeff","").strip().lower(): (v.strip() if isinstance(v,str) else v)
    ...
    with engine.begin() as cx, open(path, newline='', encoding='utf-8') as f:
        for raw in csv.DictReader(f):
            ...
            """), {
                ...
                "lat": _num(r.get("lat")),
                "lon": _num(r.get("lon")),
                "amenities_json": json.dumps(json.loads(r.get("amenities_json","{}")) if r.get("amenities_json") else {})
            })
...
def load_rates(path):
    with engine.begin() as cx, open(path, newline='', encoding='utf-8') as f:
        for raw in csv.DictReader(f):
            ...
            """), {
                "hotel_id": r.get("hotel_id") or r.get("hotelid"),
                "room_type": r.get("room_type") or r.get("roomtype") or r.get("type"),
                "occupancy": int(r.get("occupancy") or r.get("guests") or 2),
                "currency": r.get("currency") or "USD",
                "base_rate": _num(r.get("base_rate") or r.get("price_per_night") or r.get("rate") or 0.0, 0.0),
                "refundable": _bool(r.get("refundable")),
                "breakfast_included": _bool(r.get("breakfast_included")),
            })

def _resolve_hotel_id(cx, hotel_name: str, city: str = None):
    if not hotel_name: return None
    if city:
        row = cx.execute(text("SELECT hotel_id FROM hotels WHERE lower(name)=lower(:n) AND lower(city)=lower(:c) LIMIT 1"),
                         {"n": hotel_name, "c": city}).fetchone()
        if row: return row[0]
    row = cx.execute(text("SELECT hotel_id FROM hotels WHERE lower(name)=lower(:n) LIMIT 1"),
                     {"n": hotel_name}).fetchone()
        ...
        for raw in csv.DictReader(f):
            ...
            if not hotel_id:
                ...
                if not hotel_id:
                    ...
                    continue
            key = r.get("key") or r.get("policy_key") or r.get("policy") or r.get("name")
            value = r.get("value") or r.get("policy_value") or r.get("details") or r.get("text")
            if not key or value is None:
                continue
            cx.execute(text("INSERT INTO policies (hotel_id,key,value) VALUES (:hotel_id,:key,:value)"),
